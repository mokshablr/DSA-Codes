//Code by Arjun R Maniyani

#include<stdio.h>
#include<stdlib.h>

//Structure of each node. Each node will contain the data to be stored as well as the link to the next node(Can be NULL if only 1 node).
typedef struct Node{
    int data;
    struct Node * link;
}NODE; //renaming to NODE so that we dont have to say "struct Node" everytime.
...
void insertIndex(NODE * start, int data, int index){
    ...
    //temp link should point to the new node
    new -> link = temp ->link;
    temp -> link = new;
}
...
//DELETING OPERATIONS
//NOTE: We will have to use the "temp" variable for *deletion* since we would have to free memory and need a way to set the link to the deleted node to NULL or to the next node.
